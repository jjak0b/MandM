...
`<div class="row" >
	<div class="col-3">
		...
		<ul id="mission-list" class="list-group overflow-auto">
			<li v-for="(mission, index) in missions" v-bind:tabindex='index+1'  v-on:click="load( mission )" v-bind:class="['list-group-item', 'list-group-item-action', missionCache == mission ? 'active' : '' ]">
				<i18n-input-widget
					v-bind:tag="'label'"
	...
	<form class="col-9" v-on:submit.prevent="save()" >
		...
		<div class="form-group btn-group" role="group">
			<button type="submit" class="form-control btn btn-success">{{ missionCache ? $t( 'shared.label-save' ) : $t( 'shared.label-add' ) }}</button>
			<button type="button" v-on:click="remove()" class="form-control btn btn-danger" v-if="missionCache != null ">{{ $t( 'shared.label-remove' ) }}</button>
		</div>
	</form>
</div>`
;
